...
// import { useRouter } from 'next/router'; // Import useRouter if necessary
    ...
    {links.map((link, index) => {
      if (!Array.isArray(link)) {
        return (
          <Link
            key={index}
            href={link.path} // Use href instead of to
...
const QuickLinks: React.FC = () => {
  ...
  const kiitGroup: NestedLinkItem[] = [
    ...
    { name: 'KIIT', path: 'https://kiit.ac.in/', isHeader: false }, // External link
    { name: 'KISS', path: '/kiss' },
    { name: 'KIMS', path: '/kims' },
